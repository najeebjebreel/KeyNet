...
def get_signed_wmcarrierset(identity_string, signature_size = 25, wmcarrierset = 'stl10'):
    ...
    if wmcarrierset == 'mnist':
        ...
        dataset = list(map(lambda x: [np.array(x[0].resize((32, 32), Image.CUBIC))/255, x[1]], dataset))
    ...
    if wmcarrierset !='stl10':
        for i in range(len(my_x)):
            ...
            my_x[i] = cv2.resize(my_x[i], dsize=(32, 32), interpolation=cv2.INTER_CUBIC)
...
def get_signed_diff_dist(identity_string, signature_size = 25):
    ...
    dataset = list(map(lambda fn: cv2.resize(plt.imread(fn), dsize = (32, 32), interpolation = cv2.INTER_CUBIC)/255, filenames))
