...
class KeyNet:
    ...
    original_classes, owner_identity_string, attacker_identity_string, signature_size, train_wm_black_box_epochs, 
    ...
    original_dataset, wmcarrierset, attacker_wmcarrierset, model_name, seed):
        ...
        self.train_wm_black_box_epochs = train_wm_black_box_epochs
        ...
        if self.original_dataset == 'cifar10':
            # Data
            print('==> Preparing data..')
            self.original_train_dataset, self.original_test_dataset, self.original_train_loader, self.original_test_loader = \
            get_flagged_cifar10_dataset(self.train_batch_size, self.test_batch_size)
            print('==> CIFAR 10 original dataset has been loaded.')
        ...
        if self.model_name == 'ResNet18':
            ...
            self.fromscratchmodel = ResNet18WM()
            ...
            self.attacker_model = ResNet18ATCRWM()
            ...
            # Combined original and watermark model, loss and optimizer to embed the watermark from scratch
            self.fromscratchmodel = VGGWM('VGG16')
            self.fromscratchmodel.to(self.device) 
            self.fromscratchcriterion = nn.CrossEntropyLoss()
            self.fromscratchcriterion2 = nn.CrossEntropyLoss(reduction='none')
            self.fromscratchoptimizer = torch.optim.SGD(self.fromscratchmodel.parameters(), lr=self.learning_rate,
            ...
            #Combined model of the plagiarizer that will be used to overwrite the owner's watermark
            self.attacker_model = VGGATCRWM('VGG16')
            self.attacker_model.to(self.device) 
            self.attacker_criterion2 = nn.CrossEntropyLoss(reduction='none')
            self.attacker_optimizer = torch.optim.SGD(self.attacker_model.parameters(), lr=self.learning_rate,
                        momentum=self.momentum)
    ...
    def train_original_task(self, epochs = None, resume = False):
        ...
        plot_loss_original(train_losses,  test_losses, title = 'Baseline loss', filename = 'cifar10_loss_original', save = True)
        plot_accuracy_original(train_accs,  test_accs, title = 'Baseline accuracy', filename = 'cifar10_acc_original', save = True)
...
# Training of the private model with a black box access for one epoch
    def train_epoch_wm_black_box(self, train_loader):
        self.wm_black_box_model.train()
        train_loss = 0
        correct = 0
        total = 0
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            self.wm_black_box_optimizer.zero_grad()
            outputs = self.wm_black_box_model(inputs).squeeze()
            loss = self.wm_black_box_criterion(outputs, targets)
            loss.backward()
            self.wm_black_box_optimizer.step()
            train_loss += loss.item()
            _, predicted = outputs.max(1)
            total += targets.size(0)
            correct += predicted.eq(targets).sum().item()
    ...
    # Testing of the private model with a black box access
    def test_BBox_private_model(self, test_loader, private_model):
        private_model.eval()
        test_loss = 0
        correct = 0
        total = 0
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(test_loader):
                inputs, targets = inputs.to(self.device), targets.to(self.device)
                targets = targets.long().squeeze()
                outputs = private_model(inputs).squeeze()
                loss = self.criterion(outputs, targets)
                test_loss += loss.item()
                _, predicted = outputs.max(1)
                total += targets.size(0)
    ...
    # Training the private model with a black box access for n epochs
    def train_private_model_with_black_box_access(self, train_wm_black_box_epochs = None, resume = False, dataset = 'black_box_cifar10'):

        wm_black_box_trainingset, wm_black_box_testingset =  self.wm_trainingset, self.wm_testingset

        wm_black_box_trainloader = torch.utils.data.DataLoader(wm_black_box_trainingset, batch_size=1, shuffle=False, num_workers=0)
        wm_black_box_testloader = torch.utils.data.DataLoader(wm_black_box_testingset, batch_size=1, shuffle=False, num_workers=0)
        ...
        wm_black_box_train_inputs, wm_black_box_train_labels = self.get_model_outputs(wm_black_box_trainloader, self.model)
        wm_black_box_test_inputs, wm_black_box_test_labels = self.get_model_outputs(wm_black_box_testloader, self.model)

        wm_black_box_trainingset = BBoxDtaset(wm_black_box_train_inputs, wm_black_box_train_labels)
        wm_black_box_testingset = BBoxDtaset(wm_black_box_test_inputs, wm_black_box_test_labels)

        wm_black_box_trainloader = torch.utils.data.DataLoader(wm_black_box_trainingset, batch_size=100, shuffle = True, num_workers = 0)
        wm_black_box_testloader = torch.utils.data.DataLoader(wm_black_box_testingset, batch_size=100, shuffle = False, num_workers = 0)
               
        if train_wm_black_box_epochs == None:
            train_wm_black_box_epochs = self.train_wm_black_box_epochs
        ...
        best_acc = 0
        for epoch in tqdm(range(train_wm_black_box_epochs)):
            train_loss, train_acc = self.train_epoch_wm_black_box(wm_black_box_trainloader)
            test_loss, test_acc = self.test_BBox_private_model(wm_black_box_testloader, self.wm_black_box_model)
            train_losses.append(train_loss)
            test_losses.append(test_loss)
            train_accs.append(train_acc)
            test_accs.append(test_acc)
        ...
        results = {
            ...
            'train_acc': train_accs,
            'test_acc': test_accs}  
        savepath = './Results/results_blackbox_'+ self.original_dataset + '_' + self.model_name + '.t7'
        torch.save(results,savepath)

        plot_loss_privatebbox(train_losses,  test_losses, title = 'Private model training in blackbox setting', filename = 'cifar10_loss_bbox_wm', save = True)
        plot_accuracy_privatebbox(train_accs,  test_accs, title = 'Private model training in blackbox setting', filename = 'cifar10_acc_bbox_wm', save = True)
    ...
    def train_epoch_private_combined(self, train_loader):
        self.seqmodel.train()
        train_loss = 0
        correct = 0
        total = 0
    ...
    def embedwm_byfinetuning(self, train_original_model_epochs = None, train_private_model_epochs = None, 
        resume = False, dataset = 'combined_wm_byfinetuning_cifar10'):
    ...
    def train_epoch_combined(self, train_loader, model_name = 'fromscratch', alpha = 0.95):
    ...
    resume = False, dataset = 'combined_fromscratch_cifar10'):
        ...
        title = 'Training from scratch of the combined model', filename = 'cifar10_acc_fromscratch', save = True)
    ...
    def finetune_original_part(self, model_name = 'fintuned', finetuning_epochs = None, data_fraction = 1.0):
        ...
        plot_accuracy_simultaneous(test_original_accs,  test_private_accs, 
        title = 'Original and private models accuracy after finetuning', filename = 'cifar10_acc_finetuning' + model_name, save = True)
...
###################################################################################################################################
 ...
 ###################################################################################################################################
    ...
    def overwrite_watermark(self, wm_overite_epochs = None, 
        ...
        plot_accuracy_simultaneous(test_original_accs,  test_private_accs, 
        title = 'Overwriting model', filename = 'cifar10_acc_overwrite_' + str(data_fraction), save = True)
